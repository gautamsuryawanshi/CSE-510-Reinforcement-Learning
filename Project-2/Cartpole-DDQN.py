...
class DQN:
    ...
    def createModel(self):
        model = Sequential()
        model.add(Dense(output_dim=64, activation='relu', input_dim=self.state_space))
        model.add(Dense(output_dim=self.action_space, activation='linear'))
    ...
    def replay(self):
        ...
        pTarget_=self.model_.predict(states_)
        ...
        for i in range(batchLen):
            ...
            a = np.argmax(predictedNextStates[i])
            
            target = predictedStates[i]
            if nextState is None:
                target[action] = reward
            else:
                #Implemnention of DDQN
                target[action] = reward + self.epsilon_decay *  pTarget_[i][ numpy.argmax(predictedNextStates[i]) ]
